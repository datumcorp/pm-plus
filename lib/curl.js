...
function parse(curl) {
    ...
    // fix: workaround for yargs not $'' aware
    const n = curl.indexOf('--data-binary')
    let newData
    if (n > -1 && curl.substr(n + 14, 10).startsWith('$')) {
        let data = curl.substr(n + 16)
    ...
    if (parsedArgs.data) {
        ...
        // newData from raw curl to bypass yargs inability to handle $''
        request.data = newData || parsedArgs['data-binary']
        request.isDataBinary = true
    } else if (parsedArgs['data-raw']) {
        // newData from raw curl to bypass yargs inability to handle $''
        request.data = newData || parsedArgs['data-raw']
    } else if (parsedArgs['d']) {
